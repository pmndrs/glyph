...
| Headless product E2E | 🟡 | A browser CLI, Vitexec, and Playwright call the same strict registry execution module; synthetic, pinned real-font direct-baker, and public loader/module-Worker lanes pass, including exact Worker-versus-Node artifact identity. Shaping, paragraph, and rendering lanes remain open. |
| Package-size lane | ✅ | Independent library-mode entries produce nonzero raw/minified/gzip/Brotli initial-core, lazy-validator, runtime-host, runtime-Worker, baker, and shaper JavaScript sizes plus raw/gzip/Brotli Wasm. Rollup static closures exclude dynamic chunks; Worker and shaper JavaScript exclude separately measured Wasm assets; future Unicode tables remain explicitly unavailable. The item-4.1 shaper baseline is 18,622 bytes minified JavaScript and 91,382 bytes optimized Wasm. |

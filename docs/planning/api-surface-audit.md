...
F10. **Stop `core/font-binding` naming every technique.** `loadedFontBindingBytes` imports `bitmap`, `msdf`, and `slug` as values and switches on all three (`packages/glyph/src/core/font-binding.ts:3`, `:60-72`), so anything reaching it pulls all three technique modules into its initial bundle graph. This is why `Paragraph` cannot move to the root entry: the delivery gate rejects the root pulling `raster/bitmap-technique.js`, and the recorded reason -- "its engine session pulls the first-party font-binding compilers" -- describes a coupling rather than a requirement. A consumer shipping one technique still pays for three. Give each technique a compile contract the binding resolves through, then re-test the move; the entry-point rule wants `Paragraph` at the root and only this blocks it.

...
F10. **A technique must carry its own font binding.** `loadedFontBindingBytes` (`packages/glyph/src/core/font-binding.ts:55-74`) is a closed `if` chain over `bitmap`, `msdf`, and `slug`, ending in `throw new TypeError('no first-party font-binding compiler is registered for ...')`. Nothing is registered -- the word describes a hard-coded branch. It is on the hot path for both entry points (`paragraph.ts:689`, `three/engine-runtime.ts:204`) and it eagerly resolves wire ids for all four first-party techniques before dispatching.

    The consequence is not primarily bundle size. **A third-party technique cannot bind a font at all.** `defineRasterTechnique` builds a pure descriptor -- `id`, `kind`, `extension`, `version` and a phantom type map -- with no binding compiler and no hook to supply one, while `compileFontBinding`, `schemaFieldTable`, `FontBindingDescriptor`, and `fontBindingResources` are all exported from `/core`. An integrator can therefore build a correct binding and has nowhere to plug it in; the call throws on their technique id. That contradicts what `/core` is for: a renderer that wants the render plan rather than our `Object3D`s is supposed to be a first-class consumer, and the example-renderer package is the standing proof of that claim. First-party techniques get a private path no external technique can reach.

    Put the compiler on the technique, so `loadedFontBindingBytes` resolves through `font.technique` -- which the call site already holds to compare ids -- instead of switching on identity. Prefer that to import-side-effect registration into a module-global registry: a registry adds order dependence, needs a `sideEffects` declaration to survive bundling, and re-creates the late-registration hazard F6 already records. Carrying the compiler on the object the caller passed in has none of those failure modes, needs no registration step, and makes first-party and third-party techniques the same shape.

...
**A rejection is our defect, not a caller mistake.** Every cause the engine can refuse a frame for is unreachable from the public API. Span offsets are validated at `set()` by `assertSpanRanges`, every surviving boundary is snapped onto the cluster grid by `alignSpansToClusters` before it reaches the engine, the tree compilers resolve the same way, and a `Text` cannot name a font that was not already loaded. So F1 is diagnostics that name which invariant this package broke and where, not a caller-actionable error taxonomy; and F2's latch is defect containment that reports one such defect once instead of failing silently at frame rate, not a recovery protocol. There is deliberately no `retry()`: nothing a caller could do would change the outcome, and offering one would put them back in charge of state they do not own.

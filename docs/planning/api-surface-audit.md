...
**A rejection is our defect, not a caller mistake.** Every cause the engine can refuse a frame for is now unreachable from the public API. Span offsets, partial overlaps, duplicate ranges, feature ranges inside a span, and unpaired surrogates are all rejected at `set()`, where the offending object can be named and the caller is still on the stack; every surviving boundary is snapped onto the cluster grid before it reaches the engine. The one refusal a caller can still provoke is `capacity.policy: 'fixed'`, which is caller-chosen behaviour -- they asked for rejection over growth -- and throws synchronously rather than producing a frame fault. A frame rejection is therefore an invariant this package broke, and the latch exists so that one such defect is reported once instead of recompiling and failing silently at frame rate behind the last good picture. There is no `retry()`: nothing a caller could do would change the outcome. F1 is therefore diagnostics naming which invariant broke, not a caller-actionable error taxonomy, and F2's latch is defect containment rather than a recovery protocol. One limitation is worth naming: partially overlapping spans are a natural way to author rich text, and refusing them is a real restriction rather than a virtue. Resolving them per cluster by cascade order would be the better surface; the engine's style scope is a stack, so that is an engine change and is recorded here as the intended end state rather than done.
...
24. Publish `@pmndrs/glyph/typegpu`, a sibling of `/tsl`: the same technique shaders realized as TypeGPU functions, reusable by any TypeGPU host without adopting our renderer. No scene integration and no engine driving, exactly as `/tsl` carries none. A TSL realization can be rendered to WGSL and GLSL in a browser probe and its final source extracted, rather than translated by inspection. An old pull request from TypeGPU's author carries a partial slug port; assume it needs reimplementation rather than resumption, but read it closely first, because it is authoritative on TypeGPU idiom. See [example renderer](example-renderer.md) for how this divides from the engine-consumer work.
    ...
    Slug shipped second, on the same terms: typed instance/addressing schemas, row- and matrix-projection vertex stages, and a canonical non-zero-winding fragment stage under `src/typegpu/slug-shader.ts`, with every styled composition (even-odd, weight boost, stem darkening, thickening) reachable through the exported band walkers and blend pieces exactly as `/tsl` reaches them at graph-build time. Parity is pinned the same way — device-free extraction of `slugShader` in both projection forms diffed per operation chain against the resolved TypeGPU WGSL — and it caught a defect an eye-translation would have shipped: the canonical fragment's unit thickening factor passed as the JS literal `1` resolves polymorphically as i32 and emits an `f32(…)` conversion at every root contribution, so the port supplies `d.f32(1)` to keep three.js's `(root * 1.0)` chain. The extraction also confirmed the header's curve count passes through f32 before its 512-curve clamp, that both fill axes keep their bounded while-loops and `-0.5` sorted-reference terminators, and that no sampler enters the page layout because every table read is an exact integer texel load. PR #46 (TypeGPU's author, reference only) got the idiom right: plain `'use gpu'` core functions over schema-typed values, slots/comptime for axis specialization, `std.select` over boolean conversions, and exact identity arguments for absent stem darkening and thickening. It went stale against the current `/tsl` realization in four places this port did not copy: its vertex half omits the y-flip that `slugShader` now applies to the quad position, outward normal, and em coordinate; it specializes the vertical solver by transposing into `solveHorizontalPolynomial` while `/tsl` keeps two axis-explicit solves (numerically identical here, so the port keeps the explicit pair); its `calcCoverage` always emits the stem-darkening term where `/tsl` branches it out at build time (behaviourally exact at zero, but a different emitted chain); and it types the whole API around `@typegpu/three` bridge nodes, which this package does not depend on.

...
| Portable Rust core | 🟡 | Validates the V0 descriptor, selects one face, rejects unsupported envelopes/tables, retains the closed shaping table profile, rebuilds checksummed SFNT bytes, derives line metrics and exact curve bounds, computes the shaping hash, emits a core GLB, and reports payload bytes. | Exercise it against the pinned authorized font and malformed corpus. |
...
| Unit verification | ✅ | Rust unit tests isolate checksum padding, quadratic/cubic extrema, and GLB alignment behavior. | Add a focused regression with every internal defect or policy branch. |
...
| TypeScript verification | 🟡 | The source-level shim passed generated/embedded ABI equality, zero-import, structured-error, and real-font-to-GLB smoke checks through Node's type stripping. The declaration build and workspace type checks remain unavailable after dependency metadata verification attempted restricted registry access. | Run the normal package build, wrapper test, and workspace type checks with the pinned pnpm dependencies installed. |

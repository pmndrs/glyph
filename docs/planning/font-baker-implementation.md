...
| Runtime shaping (milestone 4) | 🟡 | The package-owned `no_std + alloc` shaper registers the exact validator-retained SFNT/extents/availability views in HarfRust-owned Wasm state through a Rust-generated direct-memory ABI. Canonical Inter proves exact 171,056-byte retention, scoped/idempotent ownership, and automatic disposal; size reporting separates the 91,382-byte optimized Wasm from its bridge. | Complete and prove shape-plan cache reuse/disposal, then add the coarse ABI and bit-for-bit corpus gate. |

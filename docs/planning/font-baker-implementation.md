...
| Product end-to-end | 🟡 | The benchmark product exercises the public loader/Worker, exact HarfRust shaping, paragraph measurement/layout, bidi, line policy, and current-uikit-shaped boundaries with deterministic Chromium and Vitexec evidence. Rendering remains unavailable until milestone 6. | Complete item 5.4 CJK universality, then add the first bitmap frame. |
...
| Paragraph measurement/layout (items 5.1–5.3) | ✅ | Unicode 17 analysis passes complete UAX #9/#14/#29 corpora; canonical Inter and Amiri evidence covers allocation-light measurement, exact positioned SoA layouts, batched unsafe-boundary reshaping, bidi, line policy, and current-uikit-shaped integration with matching Node/Chromium/Vitexec hashes. | Add item 5.4 focused CJK shaping and horizontal paragraph universality without renderer or paging work. |

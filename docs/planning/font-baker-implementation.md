...
| Roadmap item 2.3 | 🟡 | Core and bitmap validators are shipped on import-isolated entries with offline pinned schemas, exact Khronos-report admission, semantic/payload checks, malformed-field matrices, KTX2 parsing, deterministic Rust-input/artifact-mutation fuzz smoke, and real-Inter round trips. The package-owned no-std bitmap core uses Fontations/Skrifa outlines, Zeno rasterization, generated direct-memory ABI, Binaryen optimization, dense 20-byte records, R8 KTX2 pages, and exact embedded/external page goldens. | Complete generic core-artifact composition, the public runtime bitmap module, and empty-registry round trip before closing 2.3. |

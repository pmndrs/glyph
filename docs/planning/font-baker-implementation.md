...
| Roadmap item 2.4 | 🟡 | The underlying discovery, core/raster bakers, validators, composer, and exact reports are executable; the public filesystem host and CLI are not yet exposed. | Add `bakeFont`, `bakeProject`, atomic output, and the thin ESM command. |

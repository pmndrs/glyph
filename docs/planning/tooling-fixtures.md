...
| --- | :---: | --- |
| Reference family selected | 🟡 | Inter Regular is a candidate, not yet an immutable fixture. |
| Exact source, version, license, and SHA-256 | ⬜ | Record all four in the manifest before committing or downloading bytes in CI. |
| Portable baker local real-font lane | 🟡 | `PMNDRS_TEXT_TEST_FONT` exercises an explicitly supplied licensed font and skips when absent. |
| Required pull-request real-font lane | ⬜ | Use the canonical checked-in fixture with no network and no environment-dependent substitution. |
| Benchmark-app product scenario | ⬜ | Run the canonical bytes through public package APIs in both interactive and headless modes. |

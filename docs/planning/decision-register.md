...
| D-109 | Slug V0 implements centered analytic outline in one specialized fill-plus-outline draw. Fill-only paint uses the original fill material and allocates no outline attributes or pipeline. Positive outline lazily adds per-instance color/width data and swaps the same mesh; restoring every width to zero restores the fill material. Fill and outline composite per glyph in ordinary painter order, matching the MSDF path rather than promising a global all-outlines-behind-all-fills layer. Mixed batches multiply zero-alpha outline contributions rather than branch per instance because paired dual-backend measurement did not establish a repeatable branch win. The search traverses every intersecting primary-axis band within the `0.05 em` width bound and checks the orthogonal axis only for quantized y-flat curves. Shadow remains unsupported, and fill/outline performance stays separately reported. | Settled for V0 |

...
The Wasm build uses `no_std + alloc`, aborting panics, and an explicit `dlmalloc` global allocator as its initial baseline. Allocation stays behind the ABI so the [allocator experiment](font-baker-allocator.md) can compare `rlsf`, `talc`, and lifecycle-specific arena alternatives without changing callers. Transport compression measurement is host work: the portable Wasm result reports authoritative raw bytes, while the Node/reporting host adds gzip and Brotli measurements.

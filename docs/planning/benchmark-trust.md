...
| 4a  | Measurement, metrics only     | `Paragraph.layout()`, `readPlannerMeasurements`                                                                                | one `layout()` reading sizes, baselines, ascent/descent and intrinsic widths, touching **no** positioned column                     | +5 %, **or any nonzero column materialization** — see below                                                                                  |
...
| 6   | Render plan read              | `RenderPlanView`, `readRenderPlanPatch` / `Resource` / `Buffer` / `Retirement`, via the GPU-free `readDrawList` reference host | one full traversal of a published plan's tables for the pinned paragraph                                                            | +5 %, or any growth in patch count or write bytes for an unchanged frame                                                                     |

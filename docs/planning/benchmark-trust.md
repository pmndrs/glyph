...
Every bench binds to **`Paragraph` from `/core`, not `Text` from `/three`**. It is the stable framework-neutral surface, it needs no scene graph, and the `/three` measurement surface is mid-rename — the existing `glyph:layout-benchmark` calls `Text.measureLayout()`, which that rename removes. Path 8 is the one exception and is worth recording as a limitation rather than hiding: a `/core`-only host cannot register a shaping font at all, because `RuntimeShaper.registerFont` requires a loader-registered font and `createTextRuntime` is exported only from the root entry. Font-binding benches therefore go through the root entry, and the audit already tracks that seam.
...
| 4a | Measurement, metrics only | `Paragraph.measure()`, `readTextEngineMeasurements` | one `measure()` reading sizes, baselines, ascent/descent and intrinsic widths, touching **no** positioned column | +5 %, **or any nonzero column materialization** — see below |
| 4b | Measurement, materialized | `measure()` then first touch of `inkBounds` or a positioned column | one `measure()` plus one materialization of the positioned query | +5 % |
...
**Two cache layers sit between the bench and the engine.** `Paragraph` memoizes each `ParagraphLayoutInspection` in a `Map` keyed by the constraint, and returns the *identical object* for a repeated equal constraint without touching Wasm at all. Behind that, Rust keeps one speculative transaction that survives a *different* constraint and dies on a text or style change. Labs calls the yielded function thousands of times, so a bench written as `p.measure(sameConstraints)` measures a `Map` lookup after its first iteration and will report picosecond-class timings with excellent stability. Every measure and layout bench must therefore invalidate deliberately — the existing [`glyph:layout-benchmark`](../../packages/glyph/scripts/benchmark-paragraph-layout.mts) already does this correctly by using a constraint value no earlier repetition used, and that discipline carries over. Where invalidation is not possible without also measuring `update()`, the bench measures the pair and says so in its name.

**The lazy columns materialize on enumeration.** The 21 positioned properties — `inkBounds`, `x`, `y`, `glyphIds`, `clusters`, the per-line arrays — are defined `enumerable: true`. Any spread, `Object.keys`, `JSON.stringify`, deep-equal, or console log of a measurement materializes all of them. Since labs requires the measured function to return a value to defeat dead-code elimination, the returned sink must be a single eager scalar such as `m.contentWidth`, never the measurement object itself. A bench that returns `m` measures the opposite of what its name claims.

**4a is a shape assertion, not only a timing one.** [Commit `ffe65e11`](../../packages/glyph/src/paragraph.ts) collapsed `measure()` and `layout()` into one call precisely so a flexbox host probing many widths for sizes alone never pays to copy arrays it does not touch. The benchmark protecting that must assert the *absence* of materialization, not merely that the call is fast: a regression here would appear as a modest constant factor that a 5 % threshold could plausibly miss on one paragraph, while being catastrophic for a host probing a hundred candidate widths. Assert `layoutRevision` is unchanged after a metrics-only measure and fail on any advance. Reading `layoutRevision` is itself safe — it does not materialize.
...
| `glyph:layout-benchmark` | same, for the public path; its invalidation-class separation is preserved. It is additionally already broken by the in-flight `/three` measure rename, which removes the `Text.measureLayout()` it calls — so it needs a rewrite regardless, and rewriting it onto `Paragraph` and labs together is cheaper than repairing it twice |

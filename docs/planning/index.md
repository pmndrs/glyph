...
- [Core text API](core-api.md) — authoritative application and integration API for fonts, measurement, engines, backends, retained plans, and renderer targets.
- [Font and engine ownership plan](font-runtime-ownership.md) — accepted breaking migration from runtime-bound loaded fonts and raw shaper hosts to immutable font assets, engine-owned backends, target-bound retained plans, deterministic leases, and renderer-owned device pools.
...
- [Renderer integration guide](../guides/renderer-integration.md) — how to declare a technique schema, author and register a render policy, drive a retained plan, consume all seven plan tables, and implement the retention and patch protocols.

...
| 11.6  |   ⬜   | Rebuild Bitmap, MTSDF, and Slug behind the Three.js `FontLoader` → `TextGroup` → `Text` surface with lazy shaper initialization, late binding, implicit batches of one, render-order inheritance/override, renderer isolation, and no exposed core handles. |  XL  | 11.5       |

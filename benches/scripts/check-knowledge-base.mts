/* @workflow { "name": "docs:check", "summary": "Validate the Open Knowledge Format agent archive under .agents/docs, including every concept source_digest.", "requirements": "The repository-pinned Node.js runtime.", "writes": "stdout" } */
/* @workflow { "name": "docs:update", "args": ["--write"], "summary": "Re-pin every package concept source_digest from the working tree, then validate the bundle.", "requirements": "The repository-pinned Node.js runtime.", "writes": ".agents/docs/packages/*.md source_digest pins and stdout" } */
...
const generator = `${skillScripts}/generate-package-digests.mjs`;
...
async function repinPackageDigests(): Promise<void> {
  ...
  const { stdout: report } = await execFileAsync(process.execPath, [generator, '..']);

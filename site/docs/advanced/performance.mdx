...
Measured in the batching example — a standalone `Text` plans one draw each (60 labels → 60 draws); the same labels
in a `TextGroup` with one material fold to 2; thirty labels in a group alternating two materials plan 30, and the
same thirty sorted into one run per material plan 2. `compositing: 'independent'` did not reduce the interleaved
case on the current tree (still 30); what folds is adjacency. */}
...
{/* prose: a paragraph seen larger than authored is the case `rasterPixelRatio` exists for; the steep-angle
scene on [Text in 3D](/docs/text/in-3d#steep-angles) is the same trade-off seen from the format side. */}

...
const editorial = ParagraphLayout.create({ wrap: 'word', align: 'justify', columns: { count: 3, gap: 0.4 } });
const column = Constraints.create({ width: { mode: 'exact', size: 9 } });
```

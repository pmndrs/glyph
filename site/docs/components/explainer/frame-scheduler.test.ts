import { describe, expect, it, vi } from 'vitest';

import { advancePooledRoots, type GlyphFrameStore } from './frame-scheduler';

describe('advancePooledRoots', () => {
  it('steps every active R3F root exactly once', () => {
    const first = vi.fn();
    const second = vi.fn();
    const stores: GlyphFrameStore[] = [
      { getState: () => ({ advance: first }) },
      { getState: () => ({ advance: second }) },
    ];

    advancePooledRoots(stores, 42);

    expect(first).toHaveBeenCalledExactlyOnceWith(42);
    expect(second).toHaveBeenCalledExactlyOnceWith(42);
  });
});

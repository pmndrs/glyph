...
import { GlyphLeasePolicy, type GlyphActivation } from './lease-policy';
...
export abstract class GlyphOffscreenRootElement extends HTMLElement {
  static observedAttributes = ['id', 'max-slots', 'idle-ttl', 'activation', 'max-dpr'];
  ...
  #leases = new GlyphLeasePolicy<GlyphProxyHandle>();
  ...
  attributeChangedCallback(name: string) {
    ...
    if (name === 'max-dpr') this.#frameSizeDirty = true;
    this.#scheduleReconcile();
  }
  ...
  /** Request a pointer-activated lease, optionally after a short hover-intent delay. */
  engage(proxy: GlyphProxyHandle, delayMs = 0) {
    if (this.activation !== 'pointer') return;
    if (delayMs > 0 && this.#intent?.proxy === proxy) return;
    this.#cancelIntent();
    if (delayMs > 0) {
  ...
  cancelIntent(proxy: GlyphProxyHandle) {
    ...
    this.#leases.engage(proxy, performance.now());
  ...
  #syncVisibility() {
    ...
    this.#leases.retain(this.#visible.keys());
  ...
  #rankVisible() {
    return this.#leases.rank(
      [...this.#visible].map(([key, value]) => ({ key, ...value })),
      this.activation,
      this.#priorityProxy,
    );
  }
  ...
  #tick = (timestamp: number) => {
    ...
    if (timestamp - this.#lastFrame >= 1000 / 60 - 1) {
      ...
      // Step every logical root once. Each root owns a render-phase presenter which renders and
      // copies immediately, before the following root overwrites the shared host surface.
      advancePooledRoots(
...
export class GlyphProxyElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.addEventListener('pointerenter', this.#sendPointerEnter);
    ...
    this.addEventListener('focus', this.#sendFocus);
  ...
  disconnectedCallback() {
    ...
    this.removeEventListener('focus', this.#sendFocus);
  ...
  #sendPointerEnter = (event: PointerEvent) => {
    this.#root?.engage(this, event.pointerType === 'mouse' ? this.#root.hoverDelay : 0);
  };

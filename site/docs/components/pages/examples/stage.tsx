...
import { AgXToneMapping, NoToneMapping, PerspectiveCamera, type WebGPURenderer } from 'three/webgpu';
...
export function ExampleStage({
  ...
  return (
    <>
      ...
      <ToneMapping lit={options.lit === true} />
...
function ToneMapping({ lit }: { readonly lit: boolean }) {
  const renderer = useThree((state) => state.gl);
  useLayoutEffect(() => {
    const previous = renderer.toneMapping;
    setToneMapping(renderer, lit ? AgXToneMapping : NoToneMapping);
    return () => setToneMapping(renderer, previous);
  }, [lit, renderer]);
  return null;
}

function setToneMapping(renderer: WebGPURenderer, toneMapping: WebGPURenderer['toneMapping']): void {
...
function PixelCamera() {
  ...
  return (
    <DreiOrthographicCamera
      ...
      makeDefault
      args={[-size.width / 2, size.width / 2, size.height / 2, -size.height / 2, -1_000, 1_000]}
      position={[0, 0, 10]}
    />
  );
}

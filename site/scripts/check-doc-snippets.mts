...
const ambient = join(site, 'scripts', 'snippet-ambient.d.ts');
...
await Promise.all(
  snippets.map((snippet) =>
    // Every fence is an isolated example, not one shared script scope. The
    // module marker also permits top-level await in focused loading examples.
    writeFile(join(staging, snippet.module), `${snippet.code}\nexport {};\n`),
  ),
);
await writeFile(join(staging, 'ambient.d.ts'), await readFile(ambient, 'utf8'));

...
describe('glyphSourceAliases', () => {
  it('resolves current public package exports to their source files', () => {
    const packageDirectory = resolve(import.meta.dirname, '../../packages/glyph');
    const aliases = glyphSourceAliases(packageDirectory);
    const specifier = '@pmndrs/glyph/react';
    const alias = aliases.find(({ find }) => find.test(specifier));

    expect(alias).toBeDefined();
    expect(specifier.replace(alias!.find, alias!.replacement)).toBe(resolve(packageDirectory, 'src/react/index.ts'));
  });
});

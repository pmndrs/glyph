...
import { HOLE_CENTER, HORIZON, PLAY_HORIZON, PLAY_OFFSET } from './content';
...
export const blackHoleActions = createActions((world) => ({
  ...
  /**
   * Open the finale, `joinedAt` seconds into its timeline. Play's hole hands over here, already open, where it is
   * and with the pull it has built; the sequence's finale opens at the centre from nothing.
   */
  openBlackHole: (joinedAt = 0) => {
    const collapse = world.get(Collapse)!;

    if (collapse.openedAt === undefined) {
      const handover = collapse.playSince !== undefined;
      world.set(Collapse, {
        openedAt: world.get(Time)!.now - joinedAt * 1000,
        playSince: undefined,

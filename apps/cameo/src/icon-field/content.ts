...
/** Hue rotations in OKLab hold lightness and chroma constant across the sheet. */
export const GEM_TONES: readonly string[] = [25, 75, 145, 195, 255, 310].map((hue) => oklch(0.7464, 0.105, hue));

/**
 * The sheets are turned so their long, repeating axis runs away from the lens: the field flows towards the viewer
 * rather than across the frame, and wraps out of sight in the distance.
 */
export const PATTERN_ANGLE = 1.28;

/** Where the sheets are centred on the floor, chosen so their extent is spent on the part of it the lens sees. */
export const PATTERN_CENTRE: readonly [x: number, y: number] = [0, 11];
...
/** How far the robot's wheels shove the cells around them, and how far that reach extends. */
export const ROBOT_PUSH = 26;
export const ROBOT_REACH = 3.4;

...
function waitForCurrentElementAttribute(selector: string, name: string, value: string): Promise<HTMLElement> {
  const current = document.querySelector<HTMLElement>(selector);
...
const renderedSizeControl = document.querySelector<HTMLInputElement>('input[type="range"][min="8"][max="96"]');
...
const layoutWidthControl = document.querySelector<HTMLInputElement>('input[type="range"][min="40"][max="100"]');
...
const reflowedLayoutWidth = await waitForChangedNumericAttribute(viewport, 'data-layout-width', initialLayoutWidth);
...
webgpuButton.click();
const restoredWebgpuViewport = await waitForReplacement(webglViewport, '[data-testid="bitmap-live-viewport"]');
...
const slugShadowControl = [...document.querySelectorAll<HTMLInputElement>('input[type="checkbox"]')].find(
  (candidate) => candidate.labels?.[0]?.textContent?.includes('Shadow') === true,
);
...
function waitForLiveViewportState(attributes: Readonly<Record<string, string>>): Promise<HTMLElement> {
  const find = (): HTMLElement | undefined =>
    [...document.querySelectorAll<HTMLElement>('[data-testid="bitmap-live-viewport"]')].find((candidateViewport) =>
      Object.entries(attributes).every(([name, value]) => candidateViewport.getAttribute(name) === value),
    );
  const current = find();
  if (current !== undefined) return Promise.resolve(current);
  return new Promise((resolve) => {
...
function waitForElementState(selector: string, attributes: Readonly<Record<string, string>>): Promise<HTMLElement> {
  const find = (): HTMLElement | undefined => {
...
function waitForChangedNumericAttribute(element: HTMLElement, name: string, previous: number): Promise<number> {

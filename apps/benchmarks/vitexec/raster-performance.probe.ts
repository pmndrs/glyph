...
for (const technique of ['bitmap', 'mtsdf', 'slug'] as const) {
  ...
  const viewport = await observeUntil(document.documentElement, () => {
    const candidate = document.querySelector<HTMLElement>('[data-testid="comparison-live-viewport"]');
...
function enabledButton(label: string, includes = false): Promise<HTMLButtonElement> {
  return observeUntil(document.documentElement, () =>
    [...document.querySelectorAll<HTMLButtonElement>('button')].find(
      (button) =>
        !button.disabled && (includes ? button.textContent?.includes(label) : button.textContent?.trim() === label),
    ),
  );
}

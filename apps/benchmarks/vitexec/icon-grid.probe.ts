...
const expectedContent = catalog.map(
  ({ codePoint, name }) => `${String.fromCodePoint(codePoint)}\n${name}`,
)
...
console.log(
  ...
  JSON.stringify({
    ...
    provenance: {
      ...
      assignmentCatalogSha256: await sha256(
        new TextEncoder().encode(JSON.stringify(expectedContent)),
      ),
    },
...
function readCatalog(candidate: unknown): {
  ...
  const sourceVersion =
    typeof source === 'object' && source !== null ? Reflect.get(source, 'version') : undefined
  const sourceUrl =
    typeof source === 'object' && source !== null ? Reflect.get(source, 'url') : undefined
  const sourceSha256 =
    typeof source === 'object' && source !== null ? Reflect.get(source, 'sha256') : undefined
  if (
...
function readAndValidateAssignments(
  viewport: HTMLElement,
  technique: Technique,
): readonly IconAssignment[] {
  const value: unknown = JSON.parse(requiredAttribute(viewport, 'data-icon-assignment-signature'))
  if (!Array.isArray(value))
    throw new TypeError(`${technique} assignment signature is not an array`)
  ...
  for (const candidate of value) {
    ...
    if (
      ...
      throw new Error(
        `${technique} assignment does not match pinned catalog index ${String(index)}`,
      )
    }
...
async function selectTechnique(technique: Technique): Promise<void> {
  const button = await observeDocument(() =>
    [...document.querySelectorAll<HTMLButtonElement>('button')].find(
      (candidate) =>
        candidate.textContent?.trim().startsWith(techniqueLabels[technique]) === true &&
        !candidate.disabled,
    ),
  )
  button.click()

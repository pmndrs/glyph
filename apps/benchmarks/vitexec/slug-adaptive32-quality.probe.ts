...
for (const backend of ['webgpu', 'webgl2'] as const) {
  for (const dpr of [1, 2] as const) {
    for (const { id: fontFixture } of ADVANCED_FONT_FIXTURES) {
      const artifact = adaptive32Manifest.artifacts.find((candidate) => candidate.fontFixture === fontFixture);
      ...
      console.log('slug-adaptive32-quality-start', backend, dpr, fontFixture);
      ...
      console.log('slug-adaptive32-quality-complete', backend, dpr, fontFixture);
    }
  }
}

console.log(
  'slug-adaptive32-quality-ready',
  JSON.stringify({
    schemaVersion: 0,
    kind: 'slug-adaptive32-quality-observation',
    experimentId: adaptive32Manifest.experimentId,
    baseCommit: adaptive32Manifest.baseCommit,

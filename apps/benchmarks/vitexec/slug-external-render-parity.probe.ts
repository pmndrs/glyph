...
if (
  !Array.isArray(expectedUrls) ||
  expectedUrls.length !== 5 ||
  expectedUrls.some((url) => typeof url !== 'string')
) {
...
for (const backend of ['webgpu', 'webgl2'] as const) {
  ...
  const [embeddedHash, externalHash] = await Promise.all([
    sha256(capture.embedded),
    sha256(capture.external),
  ])

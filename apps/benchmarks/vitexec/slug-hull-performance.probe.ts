...
const manifestPath = '/fixtures/autoresearch/slug-hull-bands-001/artifacts-v0.json'
...
const [
  ...
  fetchJson<HullManifest>(manifestPath),
])
...
for (const backend of ['webgpu', 'webgl2'] as const) {
  for (const fontFixture of FONT_FIXTURES) {
    ...
    if (artifact === undefined) throw new Error(`${fontFixture} lacks a hull artifact`)
    const hull = {
      url: new URL(`/fixtures/autoresearch/slug-hull-bands-001/${artifact.file}`, location.origin)
        .href,
      compressed: artifact.compressed,
      uncompressed: artifact.uncompressed,
    }
    for (let round = 0; round < ROUND_COUNT; round += 1) {
      const order: readonly Variant[] = round % 2 === 0 ? ['fixed16', 'hull'] : ['hull', 'fixed16']
      for (const variant of order) {
        const id = `${backend}-${fontFixture}-r${round}-${variant}`
        console.log('slug-hull-performance-start', id)
        const canvas = document.createElement('canvas')
        canvas.dataset.slugHullPerformanceRun = id
        ...
        try {
          ...
          preview = await createComparisonWorkloadPreview({
            ...
            ...(variant === 'hull' ? { slugBakedArtifact: hull } : {}),
            technique: 'slug',
        ...
        console.log('slug-hull-performance-complete', id)
      }
    }
  }
}
...
console.log(
  'slug-hull-performance-ready',
  JSON.stringify({
    schemaVersion: 0,
    kind: 'slug-hull-performance-observation',
...
function assertStats(
  ...
  if (
    ...
    throw new Error(`${backend} hull run did not preserve the requested product contract`)
  }
}

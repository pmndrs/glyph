...
const hullManifestPath = '/fixtures/autoresearch/slug-hull-bands-001/artifacts-v0.json'
const baselinePath = '/fixtures/results/slug-quality-matrix-chromium149.json'
const [
  { captureSlugTextConformance },
  { environmentResource },
  ...
  fetchJson<HullManifest>(hullManifestPath),
...
for (const backend of ['webgpu', 'webgl2'] as const) {
  for (const dpr of [1, 2] as const) {
    for (const { id: fontFixture } of ADVANCED_FONT_FIXTURES) {
      const artifact = hullManifest.artifacts.find(
      ...
      console.log('slug-hull-quality-start', backend, dpr, fontFixture)
      const capture = await captureSlugTextConformance({
        backend,
        bakedArtifact: {
          url: new URL(
            `/fixtures/autoresearch/slug-hull-bands-001/${artifact.file}`,
            location.origin,
          ).href,
      ...
      console.log('slug-hull-quality-complete', backend, dpr, fontFixture)
    }
  }
}

console.log(
  'slug-hull-quality-ready',
  JSON.stringify({
    schemaVersion: 0,
    kind: 'slug-hull-quality-observation',
    experimentId: hullManifest.experimentId,
    baseCommit: hullManifest.baseCommit,

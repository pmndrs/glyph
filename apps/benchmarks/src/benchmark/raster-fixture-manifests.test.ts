...
describe('checked raster fixture manifests', () => {
  ...
  it('authenticates every complete MTSDF font artifact and page total', async () => {
    expect(mtsdfManifest.artifacts.map(({ fontFixture }) => fontFixture)).toEqual(
      Object.keys(fixtureIdentities),
    )
    for (const artifact of mtsdfManifest.artifacts) {
      ...
      expect(integerProperty(raster, 'glyphCount', artifact.file)).toBe(identity.glyphCount)
      expect(arrayProperty(raster, 'pages', artifact.file)).toHaveLength(
...
async function expectCompleteFont(
  ...
  const font = objectProperty(
    objectProperty(document, 'extensions', fixtureId),
    'PMNDRS_font',
    fixtureId,
  )
  ...
  for await (const chunkValue of gunzip) {
    ...
    while (chunkOffset < chunk.byteLength && prefix.byteLength < requiredPrefixBytes) {
      const copyBytes = Math.min(
        ...
        requiredPrefixBytes - prefix.byteLength,
      )

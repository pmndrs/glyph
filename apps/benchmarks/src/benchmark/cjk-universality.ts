...
const paragraphConstraints = cjkContract.constraints as Readonly<
  Record<string, ParagraphConstraints>
>
...
export const cjkUniversalityTarget: BenchmarkTarget = {
  ...
  load: async () => {
    ...
    if (
      artifact === undefined ||
      artifact.sha256 !== cjkManifest.bake.expectedCore.artifactSha256
    ) {
  ...
  run: async () => {
    ...
    return {
      bytes:
        shapedFixtureBytes(shaped) + layouts.reduce((sum, layout) => sum + layoutBytes(layout), 0),
...
function observeShaper(
  runtime: RuntimeShaper,
  calls: { shape: number; reshape: number },
): RuntimeShaper {
  return {

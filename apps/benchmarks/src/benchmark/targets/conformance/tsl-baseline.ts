...
type BaselineState = { readonly kind: 'empty' } | { readonly kind: 'ready'; readonly resources: BaselineResources };
...
async function renderBaseline(resources: BaselineResources): Promise<TargetRunOutput> {
  ...
  const pixels = await renderer.readRenderTargetPixelsAsync(target, 0, 0, physicalSize, physicalSize);

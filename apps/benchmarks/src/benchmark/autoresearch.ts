...
export function assertAutoresearchBaseline(
  value: unknown,
): asserts value is AutoresearchBaselineV0 {
  ...
  if (!Array.isArray(value.evidence) || value.evidence.length === 0)
    invalid('evidence must be non-empty')
  ...
  for (const entry of value.evidence) {
    ...
    if (!isHexDigest(entry.sha256, 64))
      invalid('evidence sha256 must contain 64 lowercase hex bytes')

...
export interface AdvancedShapingCase {
  ...
  readonly fontFixture: "inter" | "amiri" | "noto-sans-devanagari" | "noto-sans-cjk";
...
export function initialAdvancedShapingState(): AdvancedShapingState {
  return { caseId: "latin-features", playing: false, tick: 0, editedText: undefined };
}
...
export function updateAdvancedShaping(
  ...
  switch (command.kind) {
    ...
    case "select-case":
      advancedShapingCase(command.caseId);
      return { caseId: command.caseId, playing: state.playing, tick: 0, editedText: undefined };
    case "edit":
      return { ...state, playing: false, editedText: command.text };
    case "restore-authored-text":
      return { ...state, tick: 0, editedText: undefined };
  }
}
...
export function advancedShapingFrame(state: AdvancedShapingState): AdvancedShapingFrame {
  ...
  return {
    ...
    text:
      state.editedText ??
      caseDefinition.revealUnits.slice(0, tick === 0 ? tickCount : tick).join(""),
    tick,
    tickCount,

    ...
    load: async () => {
      ...
      try {
        for (const definition of ADVANCED_SHAPING_CASES) {
          ...
          if (!response.ok) {
            throw new Error(
              `Unable to load ${definition.fontFixture} bitmap fixture (${response.status})`,
            )
          }
          fonts.set(
            definition.fontFixture,
            await registry.registerAsset(new Uint8Array(await response.arrayBuffer())),
          )
        }
      ...
      for (const definition of ADVANCED_SHAPING_CASES) {
        ...
        try {
          for (const frame of caseFrames) {
            ...
            if (layout === undefined)
              throw new Error(`${definition.id}:${frame.tick} has no layout`)
            const rendered = renderedGlyphs(text)
            const draws = bitmapDraws(text)
            const missing = layout.glyphIds.reduce(

...
export function createWebGl2FrameTimer(
  context: WebGL2RenderingContext,
  options: FrameTimerOptions,
): GpuFrameTimer {
  ...
  return {
    ...
    poll() {
      ...
      for (const measurement of pending) {
        try {
          if (
            context.getQueryParameter(measurement.query, context.QUERY_RESULT_AVAILABLE) !== true
          ) {
            waiting.push(measurement)
            continue
          }
          const nanoseconds: unknown = context.getQueryParameter(
            measurement.query,

...
describe('WebGPU frame timer', () => {
  it('queues a resolved duration under the frame that initiated resolution', async () => {
    ...
    const resolveTimestampsAsync = vi.fn<() => Promise<number | undefined>>(
      () => resolution.promise,
    )
...
function webGlHarness(options?: { readonly allocation?: boolean; readonly extension?: boolean }): {
  ...
  harness.context = {
    ...
    getExtension: vi.fn<() => typeof extension | null>(() =>
      options?.extension === false ? null : extension,
    ),
    ...
    getQueryParameter: vi.fn<(query: WebGLQuery, parameter: number) => unknown>(
      (query, parameter) => {

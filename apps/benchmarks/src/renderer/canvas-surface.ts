...
const CANVAS_GRID_COLOR = 0x3f3f4a
...
export function createCanvasGridPositions(width: number, height: number): Float32Array {
  ...
  const horizontalCount = Math.max(
    0,
    Math.ceil((height - GRID_ORIGIN_CSS_PX) / GRID_SPACING_CSS_PX),
  )
  ...
  for (let x = GRID_ORIGIN_CSS_PX; x < width; x += GRID_SPACING_CSS_PX) {
    offset = writeQuad(
      ...
      x,
      0,
      Math.min(x + GRID_LINE_WIDTH_CSS_PX, width),
      -height,
    )
  }
  for (let y = GRID_ORIGIN_CSS_PX; y < height; y += GRID_SPACING_CSS_PX) {
    offset = writeQuad(
      ...
      -y,
      width,
      -Math.min(y + GRID_LINE_WIDTH_CSS_PX, height),
    )
  }
  return positions
}
...
function createCanvasGridGeometry(width: number, height: number): THREE.BufferGeometry {
  ...
  geometry.setAttribute(
    'position',
    new THREE.BufferAttribute(createCanvasGridPositions(width, height), 3),
  )
  return geometry
}

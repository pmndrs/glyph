...
const evidenceFiles = [
  ...
  ['advanced-shaping-conformance', 'apps/benchmarks/fixtures/results/advanced-shaping-conformance-chromium149.json'],
  ['advanced-shaping-performance', 'apps/benchmarks/fixtures/results/advanced-shaping-performance-chromium149.json'],
  ...
  ['slug-performance-matrix', 'apps/benchmarks/fixtures/results/slug-performance-matrix-chromium149.json'],
  ['slug-role-scenes', 'apps/benchmarks/fixtures/results/slug-role-scenes-chromium149.json'],
  ['slug-external-render-parity', 'apps/benchmarks/fixtures/results/slug-external-render-parity-chromium149.json'],
  ['icon-grid-retained-evidence', 'apps/benchmarks/fixtures/results/icon-grid-retained-evidence-chromium149.json'],
] as const;
...
const baseline: AutoresearchBaselineV0 = {
  ...
  campaign: {
    state: 'disabled',
    reason: 'Optimization campaigns remain disabled until the release raster baselines are accepted.',
  },
  environment: {
...
if (process.argv.includes('--check')) {
  if ((await readFile(output, 'utf8')) !== serialized) {
    throw new Error(`autoresearch baseline is stale; run the generator from ${fileURLToPath(appDirectory)}`);
  }
} else {

...
const output = new URL('../fixtures/autoresearch/slug-adaptive32-bands-001/quality-chromium149.json', import.meta.url);
const child = spawn(executable, ['--gpu', '--path', '/?runner=probe', './vitexec/slug-adaptive32-quality.probe.ts'], {
  cwd,
...
if (code !== 0 || transcript.includes('[error]')) {
  throw new Error(`adaptive-32 quality capture failed with status ${String(code)}`);
}
const marker = /slug-adaptive32-quality-ready (\{[^\n]+\})/.exec(transcript)?.[1];
if (marker === undefined) throw new Error('adaptive-32 quality capture omitted its result marker');
...
function assertObservation(candidate: unknown): asserts candidate is Record<string, unknown> {
  const observation = objectRecord(candidate, 'adaptive-32 quality observation');
  if (
    observation.schemaVersion !== 0 ||
    observation.kind !== 'slug-adaptive32-quality-observation' ||
    observation.experimentId !== 'slug-adaptive32-bands-001' ||
    ...
    throw new TypeError('adaptive-32 quality observation has an invalid envelope');
  }
  const identities = new Set<string>();
  for (const candidateEntry of observation.observations) {
    const entry = objectRecord(candidateEntry, 'adaptive-32 quality entry');
    ...
    if (
      ...
      throw new TypeError(`invalid or duplicate adaptive-32 quality entry ${identity}`);
    }

...
const output = new URL(
  '../fixtures/results/slug-performance-matrix-chromium149.json',
  import.meta.url,
)
...
const child = spawn(
  executable,
  ['--gpu', '--path', '/?runner=probe', './vitexec/slug-performance-matrix.probe.ts'],
...
function assertObservation(candidate: unknown): asserts candidate is Record<string, unknown> {
  ...
  const expectedIds = new Set(
    EXPECTED_BACKENDS.flatMap((backend) =>
      EXPECTED_DPRS.flatMap((dpr) =>
        EXPECTED_FONT_FIXTURES.map((fontFixture) => `${backend}-${dpr}x-${fontFixture}`),
      ),
    ),
  )
  ...
  for (const candidateCase of observation.cases) {
    ...
    if (
      ...
      !EXPECTED_FONT_FIXTURES.includes(
        entry.fontFixture as (typeof EXPECTED_FONT_FIXTURES)[number],
      ) ||
  ...
  if (observedIds.size !== expectedIds.size) {
    throw new TypeError(
      `Slug performance matrix retained ${observedIds.size} of ${expectedIds.size} required cases`,
    )
  }
}

...
const output = new URL(
  '../fixtures/autoresearch/slug-hull-bands-001/performance-chromium149.json',
  import.meta.url,
)
const child = spawn(
  executable,
  ['--gpu', '--path', '/?runner=probe', './vitexec/slug-hull-performance.probe.ts'],
...
if (code !== 0 || transcript.includes('[error]')) {
  throw new Error(`hull performance capture failed with status ${String(code)}`)
}
const marker = /slug-hull-performance-ready (\{[^\n]+\})/.exec(transcript)?.[1]
if (marker === undefined) throw new Error('hull performance capture omitted its result marker')
...
function assertObservation(candidate: unknown): Record<string, unknown> & {
  ...
  const candidateObservation = objectRecord(candidate, 'hull performance observation')
  if (
    candidateObservation.schemaVersion !== 0 ||
    candidateObservation.kind !== 'slug-hull-performance-observation' ||
    candidateObservation.experimentId !== 'slug-hull-bands-001' ||
    ...
    throw new TypeError('hull performance observation has an invalid envelope')
  }
  const runs: CapturedRun[] = []
  const ids = new Set<string>()
  for (const candidateRun of candidateObservation.runs) {
    const run = objectRecord(candidateRun, 'hull performance run')
    const stats = objectRecord(run.stats, 'hull run stats')
    if (
      ...
      (run.variant !== 'fixed16' && run.variant !== 'hull') ||
      ...
      throw new TypeError('hull performance run identity or stats are invalid')
    }
    ids.add(run.id)
...
function summarize(runs: readonly CapturedRun[]): readonly Record<string, unknown>[] {
  ...
  for (const backend of ['webgpu', 'webgl2'] as const) {
    for (const fontFixture of FONT_FIXTURES) {
      ...
      const hull = selected.filter((run) => run.variant === 'hull')
      const pairedGpuPercent = fixed16.map((base) => {
        const candidate = hull.find((run) => run.round === base.round)
        if (candidate === undefined) throw new Error('hull run lacks its paired baseline')

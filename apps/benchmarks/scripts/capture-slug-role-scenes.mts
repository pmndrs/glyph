...
function assertObservation(candidate: unknown): asserts candidate is Record<string, unknown> {
  ...
  for (const candidateObservation of record.observations) {
    ...
    if (
      ...
      (expectedScene.viewportClipped !== undefined &&
        observation.viewportClipped !== expectedScene.viewportClipped)
    ) {
  ...
  if (observed.size !== expected.size)
    throw new TypeError('Slug role-scene observation is incomplete')
}

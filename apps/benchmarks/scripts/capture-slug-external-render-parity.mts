...
const output = new URL(
  '../fixtures/results/slug-external-render-parity-chromium149.json',
  import.meta.url,
)
...
try {
  ...
  const expectedUrls = report.execution.outputs.map(
    ({ file }) => `${servedDirectory}/${basename(file)}`,
  )
...
function assertObservation(candidate: unknown): asserts candidate is Record<string, unknown> {
  ...
  for (const candidateCase of observation.cases) {
    ...
    if (
      ['raster', 'curve', 'headers', 'references'].some(
        (resource) => sourceTypes[resource] !== 'external',
      )
    ) {
    ...
    for (const url of artifactUrls) {
      const expectedCount = /(?:-curves\.ktx2|-headers\.r32ui\.bin|-references\.r16ui\.bin)$/u.test(
        url,
      )
        ? 2
        : 1

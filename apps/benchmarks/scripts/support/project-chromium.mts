...
export async function launchProjectChromium(options: ProjectChromiumLaunchOptions): Promise<Browser> {
  const launchOptions = projectChromiumLaunchOptions(options);
  const browser = await chromium.launch(launchOptions);
  const source =
    launchOptions.executablePath === undefined ? 'Playwright-managed executable' : launchOptions.executablePath;

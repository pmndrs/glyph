...
const output = new URL(
  '../fixtures/autoresearch/slug-adaptive32-bands-001/performance-chromium149.json',
  import.meta.url,
);
const child = spawn(
  executable,
  ['--gpu', '--path', '/?runner=probe', './vitexec/slug-adaptive32-performance.probe.ts'],
...
if (code !== 0 || transcript.includes('[error]')) {
  throw new Error(`adaptive-32 performance capture failed with status ${String(code)}`);
}
const marker = /slug-adaptive32-performance-ready (\{[^\n]+\})/.exec(transcript)?.[1];
if (marker === undefined) throw new Error('adaptive-32 performance capture omitted its result marker');
...
function assertObservation(candidate: unknown): Record<string, unknown> & {
  ...
  const candidateObservation = objectRecord(candidate, 'adaptive-32 performance observation');
  if (
    candidateObservation.schemaVersion !== 0 ||
    candidateObservation.kind !== 'slug-adaptive32-performance-observation' ||
    candidateObservation.experimentId !== 'slug-adaptive32-bands-001' ||
    ...
    throw new TypeError('adaptive-32 performance observation has an invalid envelope');
  }
  const runs: CapturedRun[] = [];
  const ids = new Set<string>();
  for (const candidateRun of candidateObservation.runs) {
    const run = objectRecord(candidateRun, 'adaptive-32 performance run');
    const stats = objectRecord(run.stats, 'adaptive-32 run stats');
    if (
      ...
      (run.variant !== 'fixed16' && run.variant !== 'adaptive32') ||
      ...
      throw new TypeError('adaptive-32 performance run identity or stats are invalid');
    }
    ids.add(run.id);
...
function summarize(runs: readonly CapturedRun[]): readonly Record<string, unknown>[] {
  ...
  for (const backend of ['webgpu', 'webgl2'] as const) {
    for (const fontFixture of FONT_FIXTURES) {
      const selected = runs.filter((run) => run.backend === backend && run.fontFixture === fontFixture);
      const fixed16 = selected.filter((run) => run.variant === 'fixed16');
      const adaptive32 = selected.filter((run) => run.variant === 'adaptive32');
      const pairedGpuPercent = fixed16.map((base) => {
        const candidate = adaptive32.find((run) => run.round === base.round);
        if (candidate === undefined) throw new Error('adaptive-32 run lacks its paired baseline');
      ...
      summaries.push({
        ...
        adaptive32MedianGpuMs: median(adaptive32.map((run) => metric(run, 'medianGpuMs'))),
        medianPairedGpuPercent: median(pairedGpuPercent),
        pairedGpuPercent,
        fixed16MedianSubmitMs: median(fixed16.map((run) => metric(run, 'medianSubmitMs'))),
        adaptive32MedianSubmitMs: median(adaptive32.map((run) => metric(run, 'medianSubmitMs'))),
        fixed16GpuBytes: metric(fixed16[0]!, 'slugGpuBytes'),
        adaptive32GpuBytes: metric(adaptive32[0]!, 'slugGpuBytes'),
        fixed16ArtifactBytes: metric(fixed16[0]!, 'artifactBytes'),
        adaptive32ArtifactBytes: metric(adaptive32[0]!, 'artifactBytes'),
      });
    }
  }
  return summaries;
...
function finiteNonNegative(candidateMetric: unknown, label: string): asserts candidateMetric is number {
  if (typeof candidateMetric !== 'number' || !Number.isFinite(candidateMetric) || candidateMetric < 0) {

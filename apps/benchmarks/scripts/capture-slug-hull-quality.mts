...
const output = new URL(
  '../fixtures/autoresearch/slug-hull-bands-001/quality-chromium149.json',
  import.meta.url,
)
const child = spawn(
  executable,
  ['--gpu', '--path', '/?runner=probe', './vitexec/slug-hull-quality.probe.ts'],
...
if (code !== 0 || transcript.includes('[error]')) {
  throw new Error(`hull quality capture failed with status ${String(code)}`)
}
const marker = /slug-hull-quality-ready (\{[^\n]+\})/.exec(transcript)?.[1]
if (marker === undefined) throw new Error('hull quality capture omitted its result marker')
...
function assertObservation(candidate: unknown): asserts candidate is Record<string, unknown> {
  const observation = objectRecord(candidate, 'hull quality observation')
  if (
    observation.schemaVersion !== 0 ||
    observation.kind !== 'slug-hull-quality-observation' ||
    observation.experimentId !== 'slug-hull-bands-001' ||
    ...
    throw new TypeError('hull quality observation has an invalid envelope')
  }
  const identities = new Set<string>()
  for (const candidateEntry of observation.observations) {
    const entry = objectRecord(candidateEntry, 'hull quality entry')
    ...
    if (
      ...
      throw new TypeError(`invalid or duplicate hull quality entry ${identity}`)
    }

...
| Portable asset/resource validation  | `raster-program.ts` and Three resource/geometry reads in `engine-plan-target.ts`.                                                            | Keep at resolve/asset admission. The command-buffer row is trusted, but referenced GLB/portable resource contents are not Rust-produced command semantics.        |
...
- `mise exec -- pnpm exec oxfmt .agents/docs/planning/fallow-duplication-audit.md --write`: passed.
- `git diff --check -- .agents/docs/planning/fallow-duplication-audit.md`: passed. The report is an untracked file in this dirty
...
- `mise exec -- pnpm scripts run docs:check`: OKF conformance passed with zero errors, and the audit has no missing local
  ...
  `.agents/docs/packages/benchmarks.md` expects source digest
  `sha256:5efc764e75a5383d1eb5bcae494744ded90a8504da0ebfb27309979940750867`, and
  `.agents/docs/packages/examples.md` expects `sha256:0dd0e8d20b53ce47c29b4b5f9b57d714cabc28491e6ba63390bc75361945a51c`.
...
- `mise exec -- pnpm exec oxfmt .agents/docs/planning/fallow-duplication-audit.md --write`: passed.
- `git diff --check -- .agents/docs/planning/fallow-duplication-audit.md` passed; a no-index check of the complete untracked
...
- `mise exec -- pnpm scripts run docs:check`: OKF conformance passed with zero errors and no missing links. The workflow
  exited 1 only because concurrent Glyph source changes left `.agents/docs/packages/glyph.md` with a stale `source_digest`; the

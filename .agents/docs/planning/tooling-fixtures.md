...
| Benchmark-app product scenario              |   ✅   | Interactive and browser-headless paths run the canonical bytes through `@pmndrs/glyph/bake`; local upload is an explicit override.                                                                                                                                 |
...
Amiri and Noto CJK now provide pinned complex-script and CJK shaping fixtures. Explicit fallback fonts and behavior remain a later milestone and do not block the first rendered paragraph.

The completed pre-render roadmap item 5.4 shaping/paragraph lane pins:

- Noto Sans CJK JP Regular 2.004 with 16,467,736 authenticated source bytes, SHA-256 `68a3fc98800b2a27b371f2fb79991daf3633bd89309d4ffaa6946fd587f375b5`, selected face index 0, 65,535 glyphs, and `cmap` formats 4/6/12/14;
- exact source HarfRust, reduced-SFNT HarfRust, and exact-version HarfBuzz structured outputs over the same UTF-16 corpus;
- deterministic Node, Chromium 149, and GPU-enabled Vitexec paragraph contracts that do not require a CJK raster artifact.
...
4. The project semantic validator checks cross-field rules that JSON Schema cannot prove: buffer-view containment and non-overlap policy, record cardinality and stride, glyph-ID width, fingerprints, reciprocal shaping/raster identity, unique bitmap strikes, texture dimensions, logical page indices, embedded/external source identity, required capabilities, and configured resource limits.
...
6. Round-trip fixtures load the resulting bytes through the real package/Worker and benchmark-app paths. A malformed corpus truncates and mutates headers, chunks, JSON fields, indexes, lengths, offsets, fingerprints, records, and payloads; every case must fail deterministically without a trap or unbounded allocation.
...
The canonical `paragraph-layout-v0.json` contract is now a golden rather than a source placeholder: it owns exact natural, 720 px, and 360 px measurements plus positioned line arrays and normalized SoA hashes. The generated `paragraph-bidi-layout-v0.json` contract extends it with two complete Amiri mixed-direction SoA layouts, nine Inter start/center/end/justify/clip/max-lines/ellipsis policy layouts, and one current-uikit-shaped final content-box layout. Its generator runs the actual baked-font registry and HarfRust paragraph stack; the package E2E independently proves Amiri source-font shaping equals shaping over the reduced SFNT extracted from the validated GLB, while pinned HarfBuzz 13 agrees field-for-field. Exact arrays, line ranges, metrics, boundary-crossing counts, and portable hashes are compared rather than updated implicitly.

The uikit-shaped fixture mirrors current pmndrs/uikit's `CustomLayouting`, `FlexNode`, and resolved size/padding/border signal flow without adding uikit or Yoga to core. It covers intrinsic `minWidth`/`minHeight` derivation; `Undefined`/unconstrained, `AtMost`, and `Exactly`; definite dimensions where measurement may be skipped; first-baseline reporting; point-scale rounding at the host boundary; and a final content box different from the candidate measurement. Repeated `measure` calls must not materialize positioned-glyph arrays. Text or shaping-policy changes invalidate measurement, while paint and raster changes do not. The final `layout` positions remain relative to the content-box origin and are translated by the host fixture. See [uikit integration](uikit-integration.md).
...
Status: ✅ roadmap items 3.1–3.3 and Milestone 3 are complete

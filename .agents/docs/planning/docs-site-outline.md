...
| orbit | getting-started/examples, text/in-3d | showcase: two crossed rings of Slug type orbit a banded greyscale planet as gyroscopes, and their letters fall on it as shadows through `maskShadowNode` over the coverage. Each ring is one paragraph shaped once, copied out with `breakApart()` and placed on a circle with `placeOnPath` — the arc position is the shaped advance 1:1, so the kerning survives, and the line is justified into the circumference by spreading the leftover across word spaces only. Bent on the CPU because Slug ignores a replaced `positionNode`; one fixed low lateral key light, because glyph quads face outward and an overhead key grazes them edge-on |
| ripple | text/materials | a passage types itself out while a `positionNode` runs a wave along it, to show that shaping and deformation never meet: the wave is a function of position and time on the GPU and costs the same whether the text is still or being retyped. MSDF, phase read from `context.shader.position` and added to `context.position` |
...
| --- | --- | --- |
| Decoration draws collapse to nothing | the decoration draw's transform-index lane holds `0x80000009`-style values (a flag bit over the index) while glyph draws hold `1, 2, 3`; `indexedTransformNodes` multiplies the raw value, reads a zero matrix, and the quad collapses. `rect` (0, 0.387, 3.61, 0.023) and the packed color `#e7ecf6` are correct. | decorations. Upstream: `60a3754a6 fix(engine): preserve decoration paint layers` and `1a0c7249a fix(engine): bind decoration transforms` (2026-09-02 23:40) land on the tree; re-verify the decorations example after the rebase. |
...
| A replaced `positionNode` drops the paragraph's placement | `context.shader.position` is the vertex before the glyph and paragraph transforms; `three/internal/material-realizer.ts#position` composes those into `context.position`, and `transformPosition` is handed only to raster programs, not to a `defineTextMaterial` context — a warp built from `shader.position` renders at the draw root, ignoring the object's world matrix. It compiles either way, so the failure is silent: the ripple material adds its displacement to `context.position` and reads only its phase from `shader.position` | ripple, orbit |
| `maxContentWidth` moves with the text it is measured from | it trims trailing spaces like a line end, so measuring a repeating line's pitch as `maxContentWidth / repeats` gives a different pitch at each repeat count; the orbit ring oscillated between two counts forever and never reached `breakApart`, leaving the un-hidden source paragraph on screen. Measure the pitch once | orbit |
...
| Hook input type | resolved in codex `b0979cf55`: `useMsdf`/`useBitmap`/`useSlug` take `FontFaceSource` (`react/msdf.ts:8`) | react/hooks | closed |

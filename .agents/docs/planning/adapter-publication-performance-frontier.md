...
normalization. React now delegates immutable semantic props to Three's canonical normalizer. Vue retains only the detached
reactive snapshot its mutable proxy contract requires, and Three adopts those package-owned records without a second
clone. The existing `withGlyphs` path already starts sparse and promotes only a repeated or explicit inspection.

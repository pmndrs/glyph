...
source_digest: 'sha256:3abb4297b8bae118a4e80a3cb1463c08b44a8633f291e0fe37ed897a7c3a972e'
...
sources:
  ...
  - id: harness-controller
    ...
    title: URL, preload transition, and live-scene controller
  - id: presentation-playback
    resource: ../../../apps/benchmarks/src/controllers/presentation-playback.tsx
    title: Presentation keyboard and timed-playback controller
  - id: workload-controls
    resource: ../../../apps/benchmarks/src/components/workload-controls.tsx
    title: Live workload and shaping controls
  - id: payload-inspector
    resource: ../../../apps/benchmarks/src/components/payload-inspector.tsx
    title: Package and GPU resource inspector
  - id: harness-scene
  ...
  - id: persistent-harness-layout
    ...
    title: Persistent renderer provider and application layout adapter
  - id: harness-layout
...
The application mounts one live activity and one persistent renderer host for the selected backend. There is no hidden conformance activity, report screen, export screen, or capture action whose visibility transition can tear down and recreate the renderer. Automated conformance targets still borrow an explicitly created renderer in their own workflows and retain their existing deterministic capture contracts.

The Vite build runs the pinned React Compiler preset and emitted production bundles import React's compiler runtime. The benchmark application uses Koota only at its application boundary. Four coherent world traits own view, layout/workload, animation, and paint controls; a fifth owns the published telemetry snapshot. The world instance lives in a dedicated module so React Fast Refresh does not allocate a replacement world. Controls stream values directly into those traits, while scene and telemetry consumers subscribe only to the traits they use; no timer debounce or parallel React control state sits between a range input and the retained scene. Koota does not enter `@pmndrs/glyph`, shaping, layout, baking, raster packages, or their public contracts.[^runtime-world]

Main and Presentation are URL-selected presentations over one continuously driven render host per route and backend generation. Workload and technique changes prepare a replacement scene while the committed scene continues rendering, then atomically swap scene resources without recreating the renderer. Compatible font changes and every runtime slider update mutate retained `Text` objects. Paragraph Stress now replaces the repeated source string on its existing `Text` and invalidates its width cache instead of rebuilding the scene; its amount slider therefore streams directly through Koota like width, font-size, animation, and paint controls. Presentation playback and shortcuts live in a dedicated controller, while render configuration, workload controls, and payload inspection are separate components rather than branches of one route controller.
...
The root `app.tsx` owns runner detection, shell Suspense, and URL route selection. `controllers/harness-controller.tsx` owns URL revisions, asset-preload transitions, and live-scene composition; `controllers/presentation-playback.tsx` owns presentation timing and keyboard behavior. Persistent renderer provisioning lives in `surfaces/harness/persistent-layout.tsx`, the live scene in `surfaces/harness/scene.tsx`, application chrome in `components/harness-layout.tsx`, and Koota binding in `components/runtime-controls.tsx`. Render configuration, workload controls, and package/GPU inspection are separate components. Executable finite work remains below `benchmark/targets/conformance`, `benchmark/targets/product`, and `benchmark/targets/measurement`, with its own workflow-owned lifecycle rather than an application route.
...
stacking state rather than reconciling the same two Texts six times. Every workload shares one root, because a paragraph
batches its own spans and no workload states a compositing mode.

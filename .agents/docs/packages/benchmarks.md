...
source_digest: 'sha256:f690888d57175b9e879a7d4a6eab7e71e301a04e6162503b919617f00e4de0ff'
...
A same-host B/A/B/A check against merged main sampled two explicit 120-frame windows for every workload and Bitmap,
MTSDF, and Slug. Across the 72 windows per revision, mean CPU p50 was unchanged at 1.136 ms; CPU p95 moved from
1.918 to 1.938 ms (+1.1%), GPU p50 from 1.464 to 1.455 ms (-0.6%), and GPU p95 from 1.702 to 1.731 ms
(+1.7%). Per-cell animation-phase and scheduler variance remained larger than those aggregate deltas, so the evidence
supports performance parity rather than a workload-specific speedup claim.

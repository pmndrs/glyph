...
| 12.2  |   🟡   | Implement staged run topology and placement, then cut the generated ABI, semantic queries, publication, and renderer realization atomically to the single retained model. Preserve existing batching/draw topology, publish only f32x2 x/y placement rows, and close the measured adoption/publication overhead before completion.                                                                                                                                                                                                                                             |  XL  | 12.1        |
...
12.2 is active with the staged run/placement model, generated ABI, Three realization, and direct-TypeGPU proof integrated;
its remaining gate is consolidated correctness/release validation plus the measured small-workload adoption overhead. The

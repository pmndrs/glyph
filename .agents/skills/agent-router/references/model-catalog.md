...
Router policy defaults every Claude and Codex model to `high`, including explicit Fable, Luna, Tera, and Sol pass-through names. A higher tier is valid only when the user explicitly requests that effort and the selected provider accepts it; model choice alone never raises effort. OpenCode routes omit the effort field.

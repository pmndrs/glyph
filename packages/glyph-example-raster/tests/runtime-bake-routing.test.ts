...
import { createFontLibrary, type RuntimeFontBakeRequest } from '@pmndrs/glyph/config/font-library';
...
test('the example raster format bakes host-side while the Worker plan stays first-party', async () => {
  ...
  const font = await library.loadFont(
    { source: `data:font/ttf;base64,${source.toString('base64')}`, baked: null },
  ...
  try {
    ...
    assert.equal(font.raster, glyphExample, 'the external raster decodes from its host-baked artifact');
  } finally {
    font.dispose();
    library.dispose();
  }

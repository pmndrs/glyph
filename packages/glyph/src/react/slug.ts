...
export interface UseSlug {
  ...
  preload(input: LoadFontInput): void;
...
export const useSlug = ((input: LoadFontInput): Font<typeof slug> => useFont(input, slug)) as UseSlug;
useSlug.preload = (input) => useFont.preload(input, slug);
useSlug.clear = (input) => useFont.clear(input, slug);

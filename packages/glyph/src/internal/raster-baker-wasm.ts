...
export interface DirectRasterBakerExports {
  ...
  readonly bake: (sourcePointer: number, sourceLength: number, requestPointer: number, requestLength: number) => number;
...
export function decodeSegmentedResponse<Kind extends string>(
  ...
  if (status !== abi.response.successStatus) throw spec.createError(parseError(metadata, spec.label));
...
export function decodeResponse<Kind extends string>(
  ...
  if (
    textDecoder.decode(response.subarray(contract.magicOffset, contract.magicOffset + contract.magic.length)) !==
    contract.magic
  ) {
...
function assertResultMetadata<Kind extends string>(
  ...
  for (const artifact of result.artifacts) {
    ...
    expectedOffset = checkedEnd(artifact.byteOffset, artifact.byteLength, artifactLength, spec.label);
  }
...
function isRasterPagePayloadReport(value: unknown, pageFormat: string): value is RasterPagePayloadReport {
  return (
    isNonArrayObject(value) &&

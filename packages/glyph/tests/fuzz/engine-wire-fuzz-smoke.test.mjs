...
async function execute(module, abi, codecCases, frameCases) {
  ...
  for (let index = 0; index < mutationCount; index += 1) {
    ...
    assert.equal(fn.createRoot(1, requestCapacity, abi.layouts.engineResult.size, 0), abi.status.ok);
    ...
    assert.equal(fn.disposeRoot(1), abi.status.ok);

    const recovery = frameBytes(abi, 3);
    assert.equal(fn.createRoot(3, recovery.byteLength, abi.layouts.engineResult.size, 0), abi.status.ok);
    ...
    assert.equal(fn.disposeRoot(3), abi.status.ok);
    outcomes.push({ codecStatus, frameStatus });
  }
  return outcomes;
}

function frameBytes(abi, rootId) {

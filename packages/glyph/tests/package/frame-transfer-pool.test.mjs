...
function transferToRoot(pool, bytes, planRevision, expectedPooledBuffer) {
  let rootPublication;
  const result = pool.transfer(bytes, { sessionId: 7, planRevision }, (message, transfer) => {
...
test('variable-size transfers use exact buckets and evict the least-recently-returned buffer', () => {
  ...
  let replacement40;
  const publication40 = pool.transfer(new Uint8Array(40), { sessionId: 7, planRevision: 5 }, (message, transfer) => {
...
test('buffer, outstanding, and pooled bounds produce call-bound results without sending rejected work', () => {
  ...
  assert.deepEqual(pool.transfer(new Uint8Array(11), { sessionId: 1, planRevision: 0 }, neverSend), {
    ok: false,
    reason: 'oversized',
  });
  transferToRoot(pool, new Uint8Array(6), 1);
  assert.deepEqual(pool.transfer(new Uint8Array(5), { sessionId: 1, planRevision: 2 }, neverSend), {
    ok: false,
    reason: 'backpressure',
  });
  transferToRoot(pool, new Uint8Array(4), 3);
  assert.deepEqual(pool.transfer(new Uint8Array(1), { sessionId: 1, planRevision: 4 }, neverSend), {
    ok: false,
    reason: 'backpressure',
  });
...
test('public call inputs and worker returns are validated before ownership changes', () => {
  ...
  assert.throws(() => pool.transfer(null, { sessionId: 1, planRevision: 0 }, () => {}), /attached Uint8Array/);
  assert.throws(
    () => pool.transfer(new Uint8Array(), { sessionId: 1, planRevision: 0 }, () => {}),
    /must not be empty/,
  );
  assert.throws(() => pool.transfer(new Uint8Array(1), null, () => {}), /metadata must be an object/);
  assert.throws(
    () => pool.transfer(new Uint8Array(1), { sessionId: 0, planRevision: 0 }, () => {}),
    /sessionId must be a positive u32/,
  );
  assert.throws(
    () => pool.transfer(new Uint8Array(1), { sessionId: 1, planRevision: 0 }, null),
  ...
  assert.throws(() => pool.transfer(detachedBytes, { sessionId: 1, planRevision: 0 }, () => {}), /attached Uint8Array/);
...
test('sender failures preserve the one-copy result variants and account for detached ownership', () => {
  ...
  const failure = pool.transfer(new Uint8Array(16), { sessionId: 1, planRevision: 1 }, () => {
    throw thrown;
  });
  assert.deepEqual(failure, { ok: false, reason: 'transfer-failed', error: thrown });

  const missingTransfer = pool.transfer(new Uint8Array(16), { sessionId: 1, planRevision: 2 }, () => {});
  ...
  const detachedFailure = pool.transfer(new Uint8Array(16), { sessionId: 1, planRevision: 3 }, (message, transfer) => {

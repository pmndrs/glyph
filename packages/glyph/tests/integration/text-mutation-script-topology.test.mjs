 ...
 * never saw the defect, because its oracle is engine-only: it asks whether `measureLayout` agrees
 * with `inspectLayout`. Both read the engine, and both were correct. This file points the
    ...
    if (shaping.ligature !== undefined) {
      ...
      test(`${where}: the fixture actually substitutes the ligature these cases depend on`, { timeout }, async () => {
        ...
        try {
          ...
          const ligatedGlyphs = ligated.nodes[0].measureLayout().glyphCount;
          const plainGlyphs = plain.nodes[0].measureLayout().glyphCount;
          assert.ok(

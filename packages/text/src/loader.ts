...
export class FontRegistry {
  ...
  constructor(options: FontRegistryOptions = {}) {
    ...
    this.#maxArtifactBytes = positiveLimit(
      options.maxArtifactBytes,
      DEFAULT_MAX_ARTIFACT_BYTES,
      "maxArtifactBytes",
    );
    this.#maxBufferViews = positiveLimit(
      options.maxBufferViews,
      DEFAULT_MAX_BUFFER_VIEWS,
      "maxBufferViews",
    );
  ...
  async _registerAsset(
    ...
    if (context.sourceBytes !== undefined && (await sha256(context.sourceBytes)) !== sourceHash) {
      throw new FontLoadError(
        ...
        "runtime source bytes do not match the baked font provenance",
      );
    }
  ...
  async attachRaster(font: RegisteredFont, bytes: ArrayBufferView): Promise<RegisteredRaster> {
    ...
    if (
      match.reference.source.type === "external" &&
      match.reference.source.artifactHash !== undefined
    ) {
    ...
    if (!response.ok) {
      return {
        ...
        error: new FontLoadError(
          "BAKED_FONT_FETCH",
          `baked font request failed with HTTP ${response.status}`,
          { url },
        ),
      };
    }
...
class RegisteredFontImpl implements RegisteredFont {
  ...
  get rasterReferences(): readonly RasterReference[] {
    this.assertActive();
    return [...getRegisteredFontData(this).rasterSources.values()].map(
      ({ reference }) => reference,
    );
  }
  ...
  async loadRaster(
    ...
    if (
      source === undefined ||
      (selection.kind !== undefined && selection.kind !== source.reference.kind)
    ) {
    ...
    for (const candidate of source.externalCandidates) {
      ...
      try {
        ...
        if (!response.ok) {
          throw new FontLoadError(
            "RASTER_FETCH",
            `raster request failed with HTTP ${response.status}`,
            { url },
          );
        }

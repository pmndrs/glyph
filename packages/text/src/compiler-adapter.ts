...
export function openCompilerProjectSnapshot(
  projectRoot: string,
  entries: readonly string[],
): CompilerProjectSnapshot {
  if (compilerVersion !== supportedTypeScriptVersion) {
    throw new Error(
      `@pmndrs/text discovery supports TypeScript ${supportedTypeScriptVersion}; received ${compilerVersion}`,
    )
  }

...
export type ReactTextProps = Omit<
  ThreeElements['group'],
  keyof TextProperties | 'children' | 'ref'
> &
...
const tokenPreloads = new WeakMap<
  FontRegistry,
  WeakMap<object, Promise<LoadedFont<AnyRasterModule, FontInput>>>
>()
...
): RegisteredFont | LoadedFont<AnyRasterModule, FontInput> =>
  use(preloadFontValue(input, options))) as UseFont
...
useFontImplementation.clear = (input): void => {
  ...
  let loadedPromise: Promise<RegisteredFont | LoadedFont<AnyRasterModule, FontInput>> | undefined =
    fontPromise
...
): Promise<LoadedFont<AnyRasterModule, FontInput>> {
...
function textFontDependencies(
  ...
  if (isRegisteredFont(font)) {
    return [
      textShaper(textRegistry(font)),
      ...(raster === undefined ? [] : [preloadRaster(font, raster)]),
    ]
  }
...
function preloadRaster(
  font: RegisteredFont,
  raster: AnyRasterInput,
): Promise<LoadedRaster<AnyRasterModule>> {
  const request = rasterRequest(raster)
...
function textPatch(
  ...
  next: TextProperties,
): TextUpdateProperties | undefined {
...
const UPDATE_PROPERTY_KEYS = CORE_PROPERTY_KEYS.filter(
  (key): key is Exclude<(typeof CORE_PROPERTY_KEYS)[number], 'font' | 'raster'> =>
    key !== 'font' && key !== 'raster',
)

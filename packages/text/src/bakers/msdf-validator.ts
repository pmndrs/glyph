...
async function validateMtsdfSemantics(
  ...
  for (let pageIndex = 0; pageIndex < pageValues.length; pageIndex += 1) {
    ...
    if (variants.length !== 1) {
      fail(
        'VARIANT_COUNT',
        'MTSDF V0 pages must contain exactly one variant',
        `${pagePath}/variants`,
      )
    }
    ...
    if (
      ...
      fail(
        'VARIANT_CONTRACT',
        'MTSDF V0 requires one lossless native RGBA8 KTX2 variant',
        variantPath,
      )
    }
  ...
  if (claimedViews.size !== views.length) {
    fail(
      ...
      'MTSDF artifact contains an unclaimed buffer view',
      '/bufferViews',
    )
  }
...
function resolveLimits(
  limits: Partial<MtsdfArtifactValidationLimits> | undefined,
): MtsdfArtifactValidationLimits {
  const resolved = {

...
export function validateRasterBufferViews(
  parsed: ParsedGlb,
  label: string,
): readonly RasterBufferView[] {
...
export function claimRasterView(
  ...
  if (claimed.has(index))
    fail('BUFFER_VIEW_ALIAS', `${label} resources must use distinct views`, path)
...
export function fail(code: string, message: string, path?: string): never {
  throw new RasterArtifactValidationError([

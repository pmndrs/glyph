...
export function validateDenseGlyphRecordTable(
  ...
  for (let glyphId = 0; glyphId < glyphCount; glyphId += 1) {
    ...
    if (resource === undefined) {
      throw new DenseGlyphRecordError(
        'RECORD_PAGE',
        'record references a missing logical page',
        glyphId,
      )
    }
    ...
    const emptyRequiredSpan =
      requireNonEmptyPlaneSpans && (planeLeft === planeRight || planeBottom === planeTop)
    ...
    if (
      ...
      throw new DenseGlyphRecordError(
        'RECORD_ATLAS_BOUNDS',
        'atlas bounds exceed their logical page',
        glyphId,
      )
    }
  }

...
before(async () => {
  root = await mkdtemp(join(tmpdir(), 'pmndrs-text-loader-'))
  ...
  incompatibleBytes = replaceAscii(
    embeddedBytes,
    '"bakerVersion":"0.0.0"',
    '"bakerVersion":"9.9.9"',
  )
...
test('canonical source forms converge on one validated baked hit without source or baker access', async () => {
  ...
  const fetch = fixtureFetch(
    new Map([['https://assets.test/fonts/Inter.font.glb?v=4', embeddedBytes]]),
    calls,
  )
  ...
  assert.deepEqual(
    Buffer.from(extracted.glyphExtentsAvailability),
    Buffer.from(validated.glyphExtentsAvailability),
  )
...
test('missing and invalid probes fall back once with deduplicated diagnostics', async () => {
  ...
  assert.equal(
    calls.filter((url) => url === 'https://assets.test/fonts/Missing.font.glb').length,
    1,
  )
...
test('baked-only failures never fetch a source or invoke fallback', async () => {
  ...
  const loader = new FontLoader({
    ...
    fetch: fixtureFetch(
      new Map([['https://assets.test/invalid.glb', new Uint8Array([1, 2, 3])]]),
      [],
    ),
    async runtimeBake() {
      bakes += 1

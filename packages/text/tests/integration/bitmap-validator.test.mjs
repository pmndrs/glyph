...
import {
  BitmapArtifactValidationError,
  validateBitmapArtifact,
} from '@pmndrs/text/bakers/bitmap/validate'
...
test('covers every required bitmap field one deletion at a time', async () => {
  ...
  const required = [
    ...
    [...root, 'glyphIdWidth'],
    [...root, 'strikes'],
    ...['ppemX', 'ppemY', 'planeUnitsPerEm', 'recordBufferView', 'recordStride', 'pages'].map(
      (field) => [...root, 'strikes', 0, field],
    ),
...
test('rejects reciprocal identity, strike, record, page, KTX2, and budget mutations', async () => {
  ...
  const present = findRecord(
    bytes,
    recordsStart,
    2937,
    (view, offset) => view.getUint16(offset + 16, true) !== 0xffff,
  )
  const absent = findRecord(
    bytes,
    recordsStart,
    2937,
    (view, offset) => view.getUint16(offset + 16, true) === 0xffff,
  ...
  wrongFeature.extensions.PMNDRS_font_bitmap.strikes[0].pages[0].variants[0].requiredFeature =
  ...
  unsupportedFormat.extensions.PMNDRS_font_bitmap.strikes[0].pages[0].variants[0].gpuFormat =
    'rgba8unorm'
  ...
  Object.assign(
    mislabeledCompressed.extensions.PMNDRS_font_bitmap.strikes[0].pages[0].variants[0],
    {
...
test('validates the pinned 65,535-glyph dense-record and multi-page boundary', async () => {
  const fixture = JSON.parse(
    await readFile(
      new URL(
        '../../../../apps/benchmarks/fixtures/contracts/max-glyph-pages-v0.json',
        import.meta.url,
      ),
      'utf8',
    ),
  ...
  const externalPages = new Map(
    pages.map((_, index) => [`max-page-${index}.ktx2`, pageArtifact.bytes]),
  )
  ...
  for (const glyphId of fixture.boundaryGlyphIds) {
    assert.equal(
      new DataView(
        result.strikes[0].records.buffer,
        result.strikes[0].records.byteOffset,
      ).getUint16(glyphId * 20 + 16, true),
...
async function rejectsWithCode(bytes, code, validationContext = context) {
  await assert.rejects(
    ...
    (error) =>
      error instanceof BitmapArtifactValidationError &&

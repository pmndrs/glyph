...
test('mirrors the current uikit CustomLayouting and resolved content-box flow', async () => {
  ...
  for (let index = 0; index < 20; index += 1) {
    assert.deepEqual(custom.measure(360, YogaMeasureMode.AtMost, 90, YogaMeasureMode.AtMost), atMost);
  }
  assert.equal(fixture.calls.layout, 0);

  const beforeDefinite = fixture.calls.measure;
  assert.deepEqual(fixture.resolveYogaLeaf(401.237, YogaMeasureMode.Exactly, 150.111, YogaMeasureMode.Exactly), {
    width: 401.24,
    height: 150.12,
    measured: false,
  });

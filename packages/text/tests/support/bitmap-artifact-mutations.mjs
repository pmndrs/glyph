export const BITMAP_ARTIFACT_FUZZ_SEED = 0x504d4e44

export function* mutateBitmapArtifact(source, runs, seed = BITMAP_ARTIFACT_FUZZ_SEED) {
  const structuralSpan = Math.min(source.byteLength, 65_536)
  let state = seed >>> 0
  const random = () => {
    state ^= state << 13
    state ^= state >>> 17
  ...
  for (let id = 0; id < runs; id += 1) {
    ...
    if (mode === 0) {
      bytes = source.slice()
      const offset = random() % structuralSpan
      bytes[offset] ^= 1 << (random() & 7)
    } else if (mode === 1) {
      bytes = source.slice(0, random() % source.byteLength)
    } else if (mode === 2) {
      bytes = source.slice()
      const offset = random() % (structuralSpan - 3)
      ...
      bytes = source.slice()
      const start = random() % structuralSpan
      const length = Math.min(1 + (random() & 31), structuralSpan - start)

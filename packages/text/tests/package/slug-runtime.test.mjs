...
test('Slug uploads exact integer resources and preserves consecutive page runs', async (context) => {
  ...
  assert.deepEqual([firstColor.getX(0), firstColor.getY(0), firstColor.getZ(0), firstColor.getW(0)], [1, 0, 0, 0.5]);
  ...
  for (const pageResource of resource.pages) {
    for (const texture of [pageResource.curveTexture, pageResource.headerTexture, pageResource.referenceTexture]) {

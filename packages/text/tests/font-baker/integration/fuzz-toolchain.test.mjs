...
test('the fuzz-only nightly exception is exact, isolated, and mise-owned', async () => {
  const [productToolchain, fuzzToolchain, fuzzMise, fuzzManifest, fuzzLock, runner] = await Promise.all([
    ...
    readFile(new URL('../../../rust/font-baker-fuzz/rust-toolchain.toml', import.meta.url), 'utf8'),
    readFile(new URL('../../../rust/font-baker-fuzz/mise.toml', import.meta.url), 'utf8'),
    readFile(new URL('../../../rust/font-baker-fuzz/Cargo.toml', import.meta.url), 'utf8'),
    readFile(new URL('../../../rust/font-baker-fuzz/Cargo.lock', import.meta.url), 'utf8'),
    readFile(new URL('../../../scripts/font-baker/fuzz-rust.mjs', import.meta.url), 'utf8'),
  ]);
  ...
  assert.match(
    fuzzManifest,
    /pmndrs-text-font-baker = \{ path = "\.\.\/font-baker", default-features = false, features = \["std"\] \}/,
  );

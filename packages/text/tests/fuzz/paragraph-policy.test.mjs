...
test('fixed-seed CJK boundary mutations stay safe, finite, and deterministic', async () => {
  ...
  assert.deepEqual(new Set(cases.map(({ style }) => style.language)), new Set(['zh-hans', 'zh-hant', 'ja', 'ko']));
  assert.deepEqual(
  ...
  for (const text of ['漢\ud800字', '漢\udc00字', '禰\udb40', '禰\udd00', '禰\udd00\udb40']) {
    assert.throws(() => paragraph.update({ text, font: font.handle, style: { language: 'ja' } }), /well-formed UTF-16/);
  }
  for (const text of ['\u{FE00}', '\u{E0100}', '漢\u{FE00}\u{FE01}', '禰\u{E0100}\u{E0101}']) {
    ...
    assert.equal(paragraph.layout({ width: { mode: 'exactly', size: 1 }, wrap: 'word' }), firstLayout);
  }

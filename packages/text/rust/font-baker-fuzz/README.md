...
```sh
pnpm --filter @pmndrs/text/bake run fuzz rust
pnpm --filter @pmndrs/text/bake run fuzz mtsdf
```

For a bounded verification run:

```sh
pnpm --filter @pmndrs/text/bake run fuzz rust -- -runs=1000 -max_len=1048576
pnpm --filter @pmndrs/text/bake run fuzz mtsdf -- -runs=1000 -max_len=1666
```

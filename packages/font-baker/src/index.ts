...
export interface FontBaker {
  bakeFont(
    source: Uint8Array,
    descriptor?: Partial<FontBakeDescriptorV0>,
  ): FontBakeResultV0
}
...
export async function createFontBaker(
  source: FontBakerWasmSource,
): Promise<FontBaker> {
...
export function createFontBakerFromInstance(
  ...
  return {
    bakeFont(source, descriptor = {}) {
      const normalized = {
        formatVersion: descriptor.formatVersion ?? 0,
        fontFaceIndex: descriptor.fontFaceIndex ?? 0,
      } satisfies FontBakeDescriptorV0
      const descriptorBytes = textEncoder.encode(JSON.stringify(normalized))
      ...
      try {
        ...
        exports.pmndrs_font_baker_dealloc(
          descriptorPointer,
          descriptorBytes.byteLength,
        )
...
export function readFontBakerAbi(
  instance: WebAssembly.Instance,
): FontBakerAbiV0 {
...
function readBootstrapFunction(
  exports: WebAssembly.Exports,
  name: string,
): () => number {
...
function decodeResponse(
  ...
  const metadataLength = view.getUint32(
    response.metadataByteLengthOffset,
    true,
  )
  const artifactLength = view.getUint32(
    response.artifactByteLengthOffset,
    true,
  )
  ...
  const metadata = JSON.parse(
    textDecoder.decode(
      bytes.subarray(
        ...
        response.payloadOffset + metadataLength,
      ),
    ),
